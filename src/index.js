...
function addShortCountryList(countrylist){
    listOfCountries = countrylist.map((country)=> `<li><img src=${country.flags.svg} alt=${country.name.official} width=20 /> <span>${country.name.official}</span></li>`).join('')
      outputBlock.insertAdjacentHTML('afterbegin', listOfCountries) 
    }

function addCountryDetails(countrylist){
    aboutCountry = countrylist.map((country)=> `<li><img src=${country.flags.svg} alt=${country.name.official} width=25 /> <span style="font-size:25px">${country.name.official}</span></li>
